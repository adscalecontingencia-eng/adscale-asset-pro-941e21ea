...
ALTER TABLE public.whatsapp_clicks ADD COLUMN IF NOT EXISTS channel text;
CREATE INDEX IF NOT EXISTS whatsapp_clicks_channel_idx ON public.whatsapp_clicks (channel);

DROP POLICY IF EXISTS "Anyone can insert clicks" ON public.whatsapp_clicks;
CREATE POLICY "Anyone can insert clicks" ON public.whatsapp_clicks
  ...
  WITH CHECK (
    ...
    AND ((channel IS NULL) OR (length(channel) <= 50))
  );
